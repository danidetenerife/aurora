import type { FetchFunction } from '@aurora/plugin-sdk';
...
const PATHFINDER_URL = decode(
  'aHR0cHM6Ly9hcGktcGFydG5lci5zcG90aWZ5LmNvbS9wYXRoZmluZGVyL3YyL3F1ZXJ5',
);
...
const OPERATION_HASHES: Record<OperationName, string> = {
  ...
  fetchPlaylist:
    'e578eda4f77aae54294a48eac85e2a42ddb203faf6ea12b3fddaec5aa32918a3',
  fetchPlaylistContents:
    'c56c706a062f82052d87fdaeeb300a258d2d54153222ef360682a0ee625284d9',
};
...
export class MetadataClient {
  ...
  async resolveArtistUri(artistUriOrName: string): Promise<string> {
    ...
    if (
      cached &&
      Date.now() - cached.fetchedAt < MetadataClient.ARTIST_CACHE_TTL_MS
    ) {
  ...
  async getArtistOverview(artistUriOrName: string): Promise<Artist> {
    ...
    if (
      cachedUri &&
      Date.now() - cachedUri.fetchedAt < MetadataClient.ARTIST_CACHE_TTL_MS
    ) {
      return cachedUri.data;
    }

    const response =
      await this.pathfinderQuery<PathfinderArtistOverviewResponse>(
        'queryArtistOverview',
        { uri: artistUri, locale: '' },
      );
    const artist = response.data.artistUnion;
    this.artistOverviewCache.set(artistUriOrName, {
      data: artist,
      fetchedAt: Date.now(),
    });
    this.artistOverviewCache.set(artistUri, {
      data: artist,
      fetchedAt: Date.now(),
    });
    return artist;
  ...
  private async executePathfinderRequest(
    body: string,
    token: string,
  ): Promise<Response> {
  ...
  private async pathfinderQuery<T>(
    ...
    if (response.status === 401) {
      ...
      const retryResponse = await this.executePathfinderRequest(
        body,
        refreshedToken,
      );
      if (!retryResponse.ok) {
