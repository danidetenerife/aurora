...
const SERVER_TIME_URL = decode(
  'aHR0cHM6Ly9vcGVuLnNwb3RpZnkuY29tL2FwaS9zZXJ2ZXItdGltZQ==',
);
...
export class AuthClient {
  ...
  private async fetchAccessToken(retryCount = 0): Promise<string> {
    if (retryCount >= MAX_SECRET_RETRIES) {
      throw new Error(
        'All TOTP secrets failed - unable to obtain access token',
      );
    }
  ...
  private async getValidSecret(): Promise<DecodedSecret> {
    if (
      this.cachedDecodedSecret &&
      !this.failedSecretVersions.has(this.cachedDecodedSecret.version)
    ) {
  ...
  private async getServerTime(): Promise<number> {
    const response = await this.fetch(SERVER_TIME_URL, {
      headers: BROWSER_HEADERS,
    });
    if (!response.ok) {
