import type {
  ...
  ArtistRef,
  ArtworkSet,
  Album as AuroraAlbum,
  Track as AuroraTrack,
  Playlist,
  PlaylistItem,
  TrackRef,
  ...
  const dateIso =
    ...
    [
      ...
      date.precision !== 'YEAR'
        ...
        : undefined,
      date.precision === 'DAY' ? String(date.day).padStart(2, '0') : undefined,
    ]
      .filter(Boolean)
      .join('-');
...
export const mapTrackToAuroraTrack = (track: SourceTrack): AuroraTrack => ({
  title: track.name,
...
export const mapTopTrackToTrackRef = (topTrack: ArtistTopTrack): TrackRef => ({
...
export const mapAlbumUnionToAlbum = (albumUnion: AlbumUnion): AuroraAlbum => ({
  title: albumUnion.name,
  artists:
    albumUnion.artists?.items?.map((artist) => ({
      name: artist.profile.name,
      roles: [],
      source: { provider: PROVIDER_ID, id: artist.uri },
    })) ?? [],
  tracks:
    albumUnion.tracksV2?.items?.map(({ track }) => ({
      title: track.name,
...
export const mapPlaylistToAuroraPlaylist = (playlist: PlaylistV2): Playlist => {
  ...
  return {
    ...
    artwork:
      playlist.images?.items?.length > 0
        ? {
    ...
    items:
      playlist.content?.items
        ?.filter(
          (contentItem) => contentItem.item?.data?.__typename === 'Track',
        )
