...
export class YtMusicClient {
  ...
  async searchSongs(query: string, resultLimit = 20): Promise<YtMusicSong[]> {
    ...
    const musicShelf = sections.find(
  ...
  async searchAlbums(query: string, resultLimit = 20): Promise<YtMusicAlbum[]> {
    const response = await this.postRequest<InnerTubeSearchResponse>('search', {
      query,
  ...
  async searchArtists(
    ...
    for (const entry of musicShelf.contents) {
      ...
      for (const run of secondaryRuns) {
        const text = run.text.trim();
        if (
          text.toLowerCase().includes('subscribers') ||
          text.toLowerCase().includes('audience')
        ) {
  ...
  async getArtist(browseId: string): Promise<YtMusicArtistDetails> {
    ...
    for (const section of sections) {
      ...
      const shelfTitle =
        ...
        carouselShelf?.header?.musicCarouselShelfBasicHeaderRenderer?.title?.runs?.[
          FIRST_INDEX
        ]?.text?.toLowerCase() ||
        '';
      ...
      playlistAuthor =
        (
          header as unknown as {
            straplineTextOne?: { runs?: Array<{ text?: string }> };
          }
        )?.straplineTextOne?.runs?.[FIRST_INDEX]?.text ??
        header.subtitle?.runs?.[FIRST_INDEX]?.text;

      const headerThumbnails =
        header.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails;
      if (headerThumbnails && headerThumbnails.length > 0) {
        playlistThumbnail = headerThumbnails[headerThumbnails.length - 1].url;
      }
    }
    ...
    for (const section of sections) {
      ...
      if (
        ...
        for (const item of items) {
          ...
          const title = twoRow.title?.runs?.[FIRST_INDEX]?.text ?? 'Playlist';
          const description = twoRow.subtitle?.runs?.[FIRST_INDEX]?.text;
          const thumbnails =
            twoRow.thumbnailRenderer?.musicThumbnailRenderer?.thumbnail
              ?.thumbnails;
          const thumbnail =
