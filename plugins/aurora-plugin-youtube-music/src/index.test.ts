...
import type { AuroraPluginAPI, YtdlpStreamInfo } from '@aurora/plugin-sdk';

import { parseDurationToMs, YtMusicClient } from './client';
import {
                      ...
                      },
                      {
                        musicCarouselShelfRenderer: {
                          header: {
                            musicCarouselShelfBasicHeaderRenderer: {
                              title: {
                                runs: [{ text: 'Fans might also like' }],
                              },
                            },
                          },
                          contents: [
  ...
  it('fetches artist bio, top tracks, albums and related artists', async () => {
    const mockClient = {
      getArtist: vi.fn().mockResolvedValue({
        ...
        name: 'The Band',
        bio: 'Band biography.',
        topTracks: [{ id: 'top1', title: 'Top Hit', artists: ['The Band'] }],
        albums: [
          {
            id: 'alb1',
            title: 'First Album',
        ...
        relatedArtists: [{ id: 'rel1', name: 'Peer Band' }],
      }),
    } as unknown as YtMusicClient;
  ...
  it('fetches top tracks, new releases, and editorial playlists', async () => {
    const mockClient = {
      getExplore: vi.fn().mockResolvedValue({
        topTracks: [{ id: 'top1', title: 'Top Song', artists: ['Top Artist'] }],
        newReleases: [
...
describe('PlaylistProvider', () => {
  ...
  it('fetches playlist by URL and converts to Aurora Playlist', async () => {
    const mockClient = {
...
describe('DiscoveryProvider', () => {
  it('returns recommendations based on previous track', async () => {
    ...
    const recommendations = await provider.getRecommendations(context, {
      variety: 1,
    });
