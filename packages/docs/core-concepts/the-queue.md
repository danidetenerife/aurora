...
The queue is your current listening session. It's the list of tracks Aurora will play through, shown in the panel on the right side of the window.
...
When a track starts playing, the streaming provider searches for matching audio and usually finds several possible results. These are the track's **stream candidates**. Aurora plays the first one that works, moving down the list if a candidate fails. The match isn't always perfect, though: you might get a live version, a remix, or a cover instead of the studio recording.
...
If the wrong version is playing, click a different candidate to switch to it. Your choice stays selected for that queue entry until its stream links expire and Aurora fetches a fresh list. Candidates that couldn't be played are marked as failed.
...
The player bar has shuffle and repeat buttons that control how Aurora moves through the queue:
...
The queue saves to disk automatically. When you close and reopen Aurora, your queue and position are restored. Tracks that were mid-stream get reset to an idle state so they can be re-fetched when you play them again.
