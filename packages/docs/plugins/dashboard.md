...
Dashboard providers supply Aurora's home screen with content like charts, trending artists, new releases, and curated playlists. When you open the player, Aurora calls each registered dashboard provider and assembles the results into an overview. Without a dashboard provider, the dashboard shows an empty state prompting the user to enable a plugin.
...
```typescript
import type { DashboardProvider, AuroraPlugin, AuroraPluginAPI } from '@aurora/plugin-sdk';
...
{% hint style="warning" %}
Always unregister your provider in `onDisable`. If you don't, it stays registered and Aurora will keep calling it after the plugin is disabled.
{% endhint %}

### Capabilities

Capabilities tell Aurora which content your provider can fetch. Aurora only calls methods for capabilities you declare. If you don't list `'topArtists'`, `fetchTopArtists` is never called even if you define it.
...
{% hint style="info" %}
Aurora only calls methods for declared capabilities. Declare only the ones you actually implement.
{% endhint %}
...
`metadataProviderId` is optional. It tells Aurora which metadata provider can look up the entities your dashboard returns.

When a user clicks an artist or album on the dashboard, Aurora navigates to a detail page. It needs to know which metadata provider can fetch that entity's full details, that's what `metadataProviderId` is for. If you omit it, clicking an artist or album sends the user to the search view instead of the detail page. If you set it to a provider that doesn't exist or can't resolve your IDs, artist and album links from the dashboard will fail.
...
When multiple dashboard providers are registered, Aurora will render results from all of them. The API wraps each provider's data in an `AttributedResult<T>`:
...
Each attributed result carries the provider's name and IDs, so Aurora can render labeled sections (e.g. "Top Tracks - Acme Music") and route navigation to the correct metadata provider.
...
All methods accept an optional `providerId`. If omitted, Aurora queries all registered dashboard providers and returns an array with one `AttributedResult` per provider. If provided, only that provider is queried.
