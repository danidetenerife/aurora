...
Theme engine utilities for Aurora. Tailwind v4 consumes CSS custom properties from `@aurora/tailwind-config/global.css`. All runtime theming is done by swapping CSS variables; no tokens are duplicated in TS.
...
Built-in basic theme IDs are namespaced with `aurora:` to avoid collisions.
