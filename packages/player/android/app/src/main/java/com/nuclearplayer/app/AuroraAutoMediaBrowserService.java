...
public class AuroraAutoMediaBrowserService extends MediaBrowserServiceCompat {
    ...
    private void initSession() {
        ...
        fallbackSession.setCallback(new MediaSessionCompat.Callback() {
            ...
            public void onPlay() {
                ...
                forwardActionToAudioService("com.nuclearplayer.ACTION_PLAY_PAUSE", true);
            }

            @Override
            public void onPause() {
                Log.i(TAG, "Auto Callback: onPause");
                forwardActionToAudioService("com.nuclearplayer.ACTION_PLAY_PAUSE", false);
            }

            @Override
            public void onSkipToNext() {
                Log.i(TAG, "Auto Callback: onSkipToNext");
                forwardActionToAudioService("com.nuclearplayer.ACTION_NEXT", null);
            }

            @Override
            public void onSkipToPrevious() {
                Log.i(TAG, "Auto Callback: onSkipToPrevious");
                forwardActionToAudioService("com.nuclearplayer.ACTION_PREVIOUS", null);
            }
    ...
    private void forwardActionToAudioService(String action, Boolean targetPlay) {
        AudioForegroundService afs = AudioForegroundService.getInstance();
        if (afs != null) {
            if ("com.nuclearplayer.ACTION_PLAY_PAUSE".equals(action)) {
                if (targetPlay != null) {
                    if (targetPlay) {
                        afs.resumeStream();
                    } else {
        ...
        if (nms != null) {
            if ("com.nuclearplayer.ACTION_NEXT".equals(action)) {
