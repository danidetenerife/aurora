...
description: Use when writing, scaffolding, or modifying Aurora plugins. Covers plugin structure, manifest, entry point, provider types, available APIs, and publishing. Trigger phrases include "create a plugin", "write a plugin", "plugin scaffold", "streaming provider", "metadata provider".
---

# Writing Aurora Plugins
...
```
my-plugin/
  package.json      # Manifest with aurora metadata
  src/
    index.ts        # Entry point, default exports AuroraPlugin
```
...
4. Submit PR to `AuroraPlayer/plugin-registry` adding your plugin to `plugins.json`
